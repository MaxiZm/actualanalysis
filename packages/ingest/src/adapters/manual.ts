    ...
    for (const filename of await files(directory)) {
      ...
      for (const result of parseResults(document)) {
        ...
        records.push(benchmarkResult({
          ...
          ...(result.protocol_id !== undefined ? { protocol_id: result.protocol_id } : {}),
          ...(result.version_inferred !== undefined ? { version_inferred: result.version_inferred } : {}),
          ...(result.metadata_incomplete !== undefined ? { metadata_incomplete: result.metadata_incomplete } : {}),
          config: result.config,
          ...(result.se !== undefined ? { se: result.se } : {}),
          ...(result.n_items !== undefined ? { n_items: result.n_items } : {}),
          ...(result.k_samples !== undefined ? { k_samples: result.k_samples } : {}),
